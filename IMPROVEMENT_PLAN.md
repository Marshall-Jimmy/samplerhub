...
| 编号 | 任务 | 优先级 | 状态 |
|------|------|--------|------|
| T0.1 | CI/CD 补强（typecheck/test/build/e2e） | T0 | 已完成 |
| T0.2 | 仓库整洁度清理 | T0 | 已完成 |
| T0.3 | Electron 安全边界显式声明 | T0 | 已完成 |
| T0.4 | Custom Protocol 路径校验加固 | T0 | 已完成 |
| T1.1 | Python Sidecar 安装方式改进 | T1 | 暂不改动 |
| T1.2 | 测试可见性增强 | T1 | 已完成 |
| T1.3 | 性能基准测试 | T1 | 待办 |
| T1.4 | DAW 集成具体化 | T1 | 已完成 |
| T2.1 | "AI-powered" 实质化（自然语言搜索） | T2 | 已完成（已有CLAP语义搜索，需持续优化） |
| T2.2 | 大库稳定性验证 | T2 | 已完成（添加9个数据库索引） |
| T2.3 | 社区冷启动 | T2 | 已完成（运营指南文档） |
| T2.4 | 文档规范化 | T2 | 已完成（CONTRIBUTING + ISSUE_TEMPLATE） |
| T3.1 | 模块化深度打磨 | T3 | 待办 |
| T3.2 | 插件系统完善 | T3 | 待办 |
| T3.3 | 跨平台打包成熟化 | T3 | 待办 |
| T3.4 | 商业化探索 | T3 | 待办 |
